...
class Opcode(object):
    ...
    def __interpret_rwrq(self, rawData):
        dataSplit = rawData.decode().replace("\x00", '')
        """
        The default folder is the one where the 'SimpleTftpServer.py' file is located
        """
        return {'filename': dir_path+dataSplit}
    @classmethod
    ...
    def __build_rwrq(self, filename, mode):
        ...
        return f"{filename}\x00{mode}\x00".encode()
    @classmethod
    ...
    def __build_error(self, code, msg):
        ...
        return struct.pack('!h', code) + msg.encode() + '\x00'.encode()
...
class FileHandler(Handler):
    ...
    def readRequest(self):
        ...
        try:
            ...
            #path = self.dataRequest['filename']
